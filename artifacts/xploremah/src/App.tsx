...
import { allRegions, allTags, destinations, type Destination } from '@/data/destinations';
  ...
  return (
    <footer className="mt-24 bg-primary text-primary-foreground">
      <div className="mx-auto grid max-w-[1240px] gap-12 px-5 py-14 md:grid-cols-[1.4fr_1fr_1fr_1fr] lg:px-8">
        ...
        <div><p className="mb-4 text-xs font-bold uppercase tracking-[0.16em] text-accent">XploreMah</p><div className="flex flex-col gap-3 text-sm text-primary-foreground/70"><Link href="/impact">Our impact</Link><Link href="/host">Become a host</Link><Link href="/#journal">Field journal</Link></div></div>
...
function DestinationCard({ destination, featured = false }: { destination: Destination; featured?: boolean }) {
  ...
  return (
    <article className={`group ${featured ? 'md:col-span-2' : ''}`} data-testid={`card-destination-${destination.id}`}>
      ...
      <div className="flex items-center justify-between px-1 pt-3"><div className="flex flex-wrap gap-1.5">{destination.tags.slice(0, 2).map((tag) => <span key={tag} className="rounded-full bg-secondary px-2.5 py-1 text-[10px] font-semibold text-muted-foreground">{tag}</span>)}</div><Link href={`/destination/${destination.id}`} className="flex items-center gap-1 text-xs font-bold text-primary opacity-0 transition-opacity group-hover:opacity-100" data-testid={`link-destination-${destination.id}`}>View stay <ArrowUpRightIcon /></Link></div>
    </article>
  );
}
...
function Impact() {
  ...
  const metrics = year === '2024' ? [{ value: '₹18.6L', label: 'shared with host communities', change: '+34%' }, { value: '1,240', label: 'native trees supported', change: '+280' }, { value: '72%', label: 'of each stay kept locally', change: 'steady' }] : [{ value: '₹13.9L', label: 'shared with host communities', change: '+28%' }, { value: '960', label: 'native trees supported', change: '+210' }, { value: '69%', label: 'of each stay kept locally', change: 'steady' }];
  return <div><section className="border-b border-border bg-secondary"><div className="mx-auto flex max-w-[1240px] flex-col gap-8 px-5 py-14 md:flex-row md:items-end md:justify-between lg:px-8 lg:py-20"><div><p className="eyebrow">The good trail</p><h1 className="mt-4 font-display text-5xl font-extrabold tracking-[-.065em] md:text-7xl">Travel that<br /><span className="text-accent-foreground">stays close.</span></h1><p className="mt-5 max-w-md text-sm leading-7 text-muted-foreground">We measure what matters after checkout: who benefits, what gets protected, and whether the welcome still feels human.</p></div><div className="flex items-center gap-2 rounded-full border border-border bg-background p-1"><button onClick={() => setYear('2023')} className={`rounded-full px-4 py-2 text-xs font-bold ${year === '2023' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground'}`} data-testid="button-impact-2023">2023</button><button onClick={() => setYear('2024')} className={`rounded-full px-4 py-2 text-xs font-bold ${year === '2024' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground'}`} data-testid="button-impact-2024">2024</button></div></div></section><div className="mx-auto max-w-[1240px] px-5 py-14 lg:px-8 lg:py-20"><div className="grid gap-4 md:grid-cols-3">{metrics.map((metric) => <div key={metric.label} className="rounded-[1.3rem] border border-border bg-card p-6"><p className="font-display text-4xl font-extrabold tracking-[-.05em] text-primary">{metric.value}</p><p className="mt-3 max-w-[160px] text-sm leading-5 text-muted-foreground">{metric.label}</p><p className="mt-7 text-xs font-bold text-accent-foreground">{metric.change} <span className="font-normal text-muted-foreground">since last year</span></p></div>)}</div><div className="mt-16 grid gap-12 lg:grid-cols-[1fr_.9fr]"><div><p className="eyebrow">Where a booking travels</p><h2 className="mt-3 font-display text-3xl font-extrabold tracking-[-.05em]">The shape of a stay</h2><p className="mt-4 max-w-md text-sm leading-6 text-muted-foreground">Of every ₹100 paid for a night on XploreMah, ₹72 moves through the host family and their nearby makers.</p><div className="mt-9 space-y-5"><ImpactBar label="Host & household" value={72} color="bg-primary" amount="₹72" /><ImpactBar label="Village projects" value={12} color="bg-accent" amount="₹12" /><ImpactBar label="Local guides & makers" value={9} color="bg-[#b5764d]" amount="₹9" /><ImpactBar label="XploreMah operations" value={7} color="bg-muted-foreground/40" amount="₹7" /></div></div><div className="rounded-[1.5rem] bg-primary p-7 text-primary-foreground md:p-9"><p className="eyebrow text-accent">This year, together</p><div className="mt-8 space-y-7"><div className="flex gap-4"><span className="grid size-10 shrink-0 place-items-center rounded-full bg-primary-foreground/10 text-accent"><TreePine size={19} /></span><div><p className="font-display text-lg font-bold">1,240 trees in the ground</p><p className="mt-1 text-xs leading-5 text-primary-foreground/60">Native jamun, hirda and ain planted with three Sahyadri villages.</p></div></div><div className="flex gap-4"><span className="grid size-10 shrink-0 place-items-center rounded-full bg-primary-foreground/10 text-accent"><Utensils size={19} /></span><div><p className="font-display text-lg font-bold">₹4.2L into local kitchens</p><p className="mt-1 text-xs leading-5 text-primary-foreground/60">Directly paid to home cooks, farms and village food collectives.</p></div></div><div className="flex gap-4"><span className="grid size-10 shrink-0 place-items-center rounded-full bg-primary-foreground/10 text-accent"><Binoculars size={19} /></span><div><p className="font-display text-lg font-bold">38 new local guides</p><p className="mt-1 text-xs leading-5 text-primary-foreground/60">Trained to lead walks that respect the forest and its seasons.</p></div></div></div></div></div><div className="mt-16 border-t border-border pt-12"><div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="eyebrow">A small promise</p><h2 className="mt-3 font-display text-3xl font-extrabold tracking-[-.05em]">Leave places better known,<br />not more crowded.</h2></div><Link href="/explore" className="inline-flex items-center gap-2 text-sm font-bold text-primary" data-testid="link-impact-explore">Choose a thoughtful stay <ArrowRight size={16} /></Link></div></div></div></div>;
}
...
function Router() {
  const [location] = useLocation();
  return <ErrorBoundary resetKey={location}><AppShell><Switch><Route path="/" component={Home} /><Route path="/explore" component={Explore} /><Route path="/destination/:id" component={DestinationPage} /><Route path="/host" component={Host} /><Route path="/impact" component={Impact} /><Route component={NotFound} /></Switch></AppShell></ErrorBoundary>;
}
